...
function getBotResponse(userMessage: string): string {
  ...
  if (message.includes("where") || message.includes("location") || message.includes("venue")) {
  ...
  if (
    ...
    return `We feature 16 dynamic sectors including: ${tradeData.sectors.slice(0, 8).join(", ")}, and more. Which sector are you interested in?`
  }

  // Benefits
  if (message.includes("why") && (message.includes("exhibit") || message.includes("participate"))) {
  ...
  // Attractions
  if (message.includes("attraction") || message.includes("feature") || message.includes("highlight")) {
    const randomAttraction = tradeData.attractions[Math.floor(Math.random() * tradeData.attractions.length)]
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-2xl bg-primary hover:bg-primary/90 z-50 hover:scale-110 transition-transform duration-700 ease-out flex items-center justify-center text-primary-foreground"
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-card border border-border rounded-lg shadow-2xl z-50 flex flex-col animate-slide-up">
          {/* Header */}
          <div className="bg-primary text-primary-foreground p-4 rounded-t-lg flex items-center justify-between">
            <div className="flex items-center gap-3">
              <MessageCircle className="h-5 w-5" />
              <div>
                <h3 className="font-semibold">IGTF Assistant</h3>
                <p className="text-xs opacity-90">Ask me anything!</p>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="text-primary-foreground hover:bg-white/10 rounded-md p-2 transition-colors duration-300"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="p-4 border-t border-border">
            <div className="flex gap-2">
              ...
              <button
                onClick={handleSend}
                className="bg-primary text-primary-foreground p-2 rounded-md hover:bg-primary/90 hover:scale-105 transition-all duration-500 w-10 h-10 flex items-center justify-center"
              >
